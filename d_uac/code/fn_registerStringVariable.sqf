...
//
//  Description:
//    This function register given string variable (from missionNamespace) to the UniAddonConf
//    system. The possible values of variable is any string.
//
//  Arguments:
...
//        your addon's name.
//    4:(func) String validation function (default={true}) --- a function which defines how
//        an appropriate string should look like. (if string given by user isn't valid it would
//        be ignored)
//
//
//  Return value:
...
//      ["my_string", "My section" "MyAddonID", "Welcome message"] call d_uac_fnc_registerStringVariable;
//
//
...
if (count _this >= 5) then
{
	[_this select 0, _this select 1, _this select 2, 
		_this select 3, CFUNC(type_string), [_this select 4] ] call CFUNC(RegisterVariable);
}
else
{
	[_this select 0, _this select 1, _this select 2, 
		_this select 3, CFUNC(type_string), [{true}] ] call CFUNC(RegisterVariable);
};
